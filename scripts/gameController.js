...
function Lerp( flAmount, flStart, flEnd ) {  return ( 1 - flAmount ) * flStart + flAmount * flEnd; }
...
function drawMovement() {
	var flCameraSpeed = 2 + Math.sqrt( accel.x * accel.x + accel.y * accel.y );
	g_flCameraX = Lerp( Math.min( 1, flFrameTime * flCameraSpeed ), g_flCameraX, offset.totalx );
	g_flCameraY = Lerp( Math.min( 1, flFrameTime * flCameraSpeed ), g_flCameraY, offset.totaly );
	...
	var flAccelStep = accel.amount * flFrameTime;
	if ( input.left === true ) { accel.x -= flAccelStep; } else if ( input.right === true ) { accel.x += flAccelStep; }
	if ( input.up === true ) { accel.y += flAccelStep; } else if ( input.down === true ) { accel.y -= flAccelStep; }

	var drag = Math.pow( 1 - FRICTION, flFrameTime * 60 ); 
	accel.x *= drag;
	accel.y *= drag;
	...
	tankpointx = ( c.width / 2 ) + ( g_flCameraX - offset.totalx );
	tankpointy = ( c.height / 2 ) + ( g_flCameraY - offset.totaly );
}
...
function drawTank() {
			...
			for (var i = 0; i < bullets.length; i += 1) {
				if ((shapes[n].x + shapes[n].size + shapes[n].accelx + (offset.totalx - bullets[i].initoffx) >= bullets[i].x + (offset.totalx - bullets[i].initoffx)) && (shapes[n].x - shapes[n].size + (offset.totalx - bullets[i].initoffx) <= bullets[i].x + bullets[i].size + (offset.totalx - bullets[i].initoffx) + shapes[n].accelx)) {
					if ((shapes[n].y + shapes[n].size + shapes[n].accely + (offset.totaly - bullets[i].initoffy) >= bullets[i].y + (offset.totaly - bullets[i].initoffy)) && (shapes[n].y - shapes[n].size + (offset.totaly - bullets[i].initoffy) <= bullets[i].y + bullets[i].size + (offset.totaly - bullets[i].initoffy) + shapes[n].accely)) {
						...
						if ( bullets[ i ].type === 2 ) dronelimit -= 1;
						if ( bullets[ i ].type === 3 ) necrolimit -= 1;
						bullets.splice( i, 1 );
						if ( bDead ) break;
					}
				}
	...
	for (var n = 0; n < bullets.length; n += 1) {
		...
		if ((bullets[n].type === 1) && (bullets[n].speed > 0)) {
			...
			bullets[n].speed *= Math.pow( 1 - FRICTION, flFrameTime * 60 );
		}
...
function keyDownHandler( e ) {
	"use strict";
	if ( e.keyCode === 65 || e.keyCode === 37 ) input.right = true;
	if ( e.keyCode === 68 || e.keyCode === 39 ) input.left = true;
	if ( e.keyCode === 87 || e.keyCode === 38 ) input.up = true;
	if ( e.keyCode === 83 || e.keyCode === 40 ) input.down = true;
	if ( e.keyCode === 69 ) {
	...
	if ( e.keyCode === 81 && editmode === true ) placeBarrel();
	if ( e.keyCode === 70 ) input.f = true;
	if ( editmode === true ) {
		if ( e.keyCode === 90 ) undo();
		if ( e.keyCode === 88 ) redo();
		if ( e.keyCode === 79 ) mirrorBarrels += 1;
		if ( e.keyCode === 80 && mirrorBarrels > 1 ) mirrorBarrels -= 1;
	}
}

function keyUpHandler( e ) {
	"use strict";
	if ( e.keyCode === 65 || e.keyCode === 37 ) input.right = false;
	if ( e.keyCode === 68 || e.keyCode === 39 ) input.left = false;
	if ( e.keyCode === 87 || e.keyCode === 38 ) input.up = false;
	if ( e.keyCode === 83 || e.keyCode === 40 ) input.down = false;
	if ( e.keyCode === 16 ) shiftheld = false;
	if ( e.keyCode === 70 ) input.f = false;
}
...
function mousedown( e ) { if ( e.button === 0 ) mouse.held = true; else mouse.rightdown = true; }

function mouseup(e) { if ( e.button === 0 ) mouse.held = false; else mouse.rightdown = false; }
...
function Main(timestamp) {
	if ( !g_flLastFrame ) g_flLastFrame = timestamp;
	flFrameTime = ( timestamp - g_flLastFrame ) / 1000;
	g_flLastFrame = timestamp;
	drawManager();
	requestAnimationFrame( Main );
}

function onload() { requestAnimationFrame( Main ); }
